...
# --------------------------------------------------
# 
# The server after being activated is just waiting for someone to contact him
# Sever is the passive one
# Adapted from https://stackabuse.com/basic-socket-programming-in-python/
# --------------------------------------------------
import socket
import dog_lib
# inicialization of the socket library
    ...
    try:  # show who connected to us
        ...
        while True:  # receive the data in small chunks (64 bytes) and print it
            ...
            if data:
                ...
                print(dog)
                first_brother = messages[4]
                second_brother = messages[5]
                # dog.addBrother(first_brother)
                # dog.addBrother(second_brother)

                for idx in range(4,len(messages)):
                    dog.addBrother(messages[idx])
            else:
